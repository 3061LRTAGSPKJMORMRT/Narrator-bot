...
module.exports = {
    ...
    run: async (message, args) => {
        ...
        return message.channel.send(`${user.id == message.author.id ? "You" : user.user.tag} currently ${user.id == message.author.id ? "have" : "has"} ${Math.floor(Math.random() * 1000)} coins ${emojis.coin}`)
    },
}
